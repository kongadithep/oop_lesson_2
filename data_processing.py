...
# my_table3_team = my_table2.select("team")

my_table1_filtered = my_table2.filter(lambda x: "ia" in str(x['team'])).filter(lambda x: int(x['minutes']) < 200).filter(lambda x: int(x['passes']) > 100)
print(my_table1_filtered)

# my_table1_filtered = table4.filter(lambda x: int(x['minutes']) < 200)
# print(my_table1_filtered)
print()
# print(my_table1.select("ranking"))
my_table1_low = my_table1.filter(lambda x: int(x['ranking']) < 10)
my_table1_high = my_table1.filter(lambda x: int(x['ranking']) >= 10)
# my_table2_low = table3.select('ranking')
print("Low rank", my_table1_low.aggregate(lambda x: sum(x)/len(x), 'games'))  #aggregate
print("High rank", my_table1_high.aggregate(lambda x: sum(x)/len(x), 'games'))

my_table2_midfielders = my_table2.filter(lambda x:(x["position"]) == "midfielder")
my_table2_forwards = my_table2.filter(lambda x:x["position"] == "forward")
...
print(f"male survive : {(len(my_table_3_survive_m.table))}")
print(f"female survise : {(len(my_table_3_survive_fm.table))}")

mytable_3_embarked = my_table_3.filter((lambda x:x['embarked'] == 'Southampton'))
my_table3_embarled_m = mytable_3_embarked.filter(lambda x:x['gender'] == "M")
print(f"embarked at Southampton : {(len(my_table3_embarled_m.table))}")
